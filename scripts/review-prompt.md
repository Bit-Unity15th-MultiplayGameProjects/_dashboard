...
- 직전 `backlogs` 의 title 은 회계 추적 키입니다. 완전히 해결됐으면 같은 title 을
  `resolved_from_backlog` 에 넣고, 아직 남았거나 부분 해결이면 같은 title 을
  `backlogs` 에 유지한 뒤 `files`/`details` 로 남은 범위를 좁힙니다. 남은 문제가
  실제로 다른 이슈로 바뀐 경우에만 기존 title 을 resolved 처리하고 새 backlog 를
  별도 추가합니다.
...
- 누적 ledger 에서 unresolved 상태로 남은 `backlogs` title 은 모두 회계 추적 키입니다.
...
- 같은 이슈의 title 변형을 두 개 이상의 open `backlogs` 로 남기지 마세요.
  ...
  하나의 backlog 로 유지합니다.
- 같은 이슈를 `todos` 와 `backlogs` 에 동시에 올리지 마세요. 이미 backlog 로
  남긴 문제라면 TODO 에서는 같은 말을 반복하지 말고, 다음 리포트까지 반드시 할
  별도 실행 단위가 있을 때만 더 좁은 title 로 작성합니다. 구분이 애매하면
  장기 미해결은 `backlogs`, 즉시 실행할 일은 `todos` 중 한쪽만 선택합니다.
...
`todos` 와 `backlogs` 는 UI 에서 함께 노출되므로 같은 이슈를 중복 표시하지
않습니다. `backlogs` 는 알려진 문제를 남기고, `todos` 는 그 문제 중 다음
리포트까지 실제로 끝낼 좁은 실행 단위만 둡니다.
이전 backlog 를 이월하기 전에는 반드시 현재 diff, 관련 `files`, 문서 스냅샷을
대조합니다. 해결됐거나 더 이상 맞지 않는 항목은 `backlogs` 에 남기지 않습니다.
단, 직전 backlog title 은 validator 가 회계 처리 여부를 검사합니다. 이전 backlog 가
미해결 또는 부분 해결이면 title 을 바꾸지 말고 그대로 유지해야 하며, 완전 해결이면
같은 title 을 `resolved_from_backlog` 로 옮깁니다.
이 규칙은 직전 backlog 뿐 아니라 전체 리포트 히스토리에서 resolved 기록 없이 남아
있는 backlog 에도 적용됩니다.
...
- 같은 이슈가 `todos` 와 `backlogs` 에 동시에 노출되지 않도록 한쪽으로
  정리했는가.
