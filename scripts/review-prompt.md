...
| `todos` | object[] | 3-10 개. `{title, files?}` 형식. 위 항목 형식 절 참조. |
| `backlogs` | object[] | 0 개 이상. 동일 형식. |
| `resolved_from_backlog` | object[] | 0 개 이상. 동일 형식. 해결된 게 없으면 `[]`. |
...
7. **`todos` / `backlogs` / `resolved_from_backlog` 는 `{title, files?}` 객체
   배열.** `title` 외에 `priority`, `effort`, `rationale` 등 추가 필드 금지
   (스키마 검증 실패). `files` 는 선택이며 repo 루트 기준 상대경로 문자열 배열.
