...
- `details` 선택, **출력 토큰 비용을 의식해 반드시 다음 룰 준수**:
  ...
  3. **`resolved_from_backlog` 항목엔 절대 쓰지 말 것** (이미 끝난 일이라
     가치 낮음 + 검증 실패).
- `title` / `files` / `details` 외 추가 필드 금지 (스키마 깨짐).
...
| `todos` | object[] | 3-10 개. `{title, files?, details?}` 형식. 위 항목 형식 절 참조. |
| `backlogs` | object[] | 0 개 이상. 동일 형식. |
| `resolved_from_backlog` | object[] | 0 개 이상. `{title, files?}` (details 금지). 해결된 게 없으면 `[]`. |
...
7. **`todos` / `backlogs` / `resolved_from_backlog` 는 `{title, files?, details?}`
   객체 배열.** 허용 키 셋이 고정 — `priority`, `effort`, `rationale` 등 다른
   이름은 추가 금지 (스키마 검증 실패). `files` 는 선택이며 repo 루트 기준
