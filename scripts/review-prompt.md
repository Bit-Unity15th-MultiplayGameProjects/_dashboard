...
## 1. 주요 변경사항
- (커밋/diff 기반 사실 나열, 3-7 개 bullet)

## 2. 코드 품질 리뷰
(평가 기준 1-3 중심으로 2-4 문단. 장점 → 우려 순. 구체적 파일/클래스 언급.)

## 3. 진행도 평가
(평가 기준 5 기준. 이전 리포트 대비, 남은 기간 고려. 1-2 문단.)

## 4. 다음 권장사항
- (우선순위 있는 bullet, 3-5 개. 각 항목은 "무엇을 왜" 형태로.
  — frontmatter `todos` 와 의미적으로 일치해야 합니다.)

## 5. 문서화 상태
(평가 기준 4 기준. `doc_scores` 각 축에 대해 "현재 상태 + 부족한 부분" 을
1-2 문장씩. 총 3-4 문단.)

## 6. Backlog
- (frontmatter `backlogs` 와 일치하는 bullet. 각 항목 "무엇이 / 왜 문제인지".)

## 7. 이전 Backlog 해결
- (frontmatter `resolved_from_backlog` 가 비어있지 않을 때만 작성.
  항목별로 "어떻게 해결됐는지" 한 줄. 비어있으면 이 섹션 자체 생략.)
```
