...
2. 이어서 **Actions** 탭 → **generate-reports** → **Run workflow** 로 첫 리포트
   ...
   적당한 팀 하나를 지정해 시작하는 편을 권장 (첫 실행에서 문제를 빠르게 관찰 가능).
...
- 특정 팀의 리뷰만 즉시 재생성: `target_repo` 만 채우고 `force=false` 로 시작.
  게이트가 막으면 `force=true` 로 재시도.
- 프롬프트 변경 후 한 팀에만 검증: `target_repo=<팀>`, `force=true`.
...
- `R` — 한 repo 가 하루에 생성하는 리포트 수.
  ...
  - 실측: 활발한 팀 2~3, 잔잔한 팀 0~1 정도. 평균 1~2 로 잡으면 현실적.
...
|---|---:|---:|---:|---:|---|
| 소규모 강의 | 8 | 2 | 16 | ~3 | Max 5x (여유) |
| 기본 (현재 예상) | 15 | 2 | 30 | ~6 | Max 5x |
| 대규모 | 30 | 3 | 90 | ~19 | Max 5x (타이트) |
| 복수 강의 병행 | 60 | 3 | 180 | ~38 | Max 20x 권장 |
